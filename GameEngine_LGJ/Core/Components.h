...
public:
	...
	void AddTile(const uint32_t x, uint32_t y, uint32_t z, bool bHasCollision)
	{
	...
	void SaveTileMap(std::string fileName)
	{
		...
		{
			std::cerr << "Error tilemap not saved " << fileName << std::endl;
		}
		saveFile.close();
	}
	...
	void LoadTileMap(std::string fileName)
	{
		...
		{
			std::cerr << "Error tilemap not loaded " << fileName << std::endl;
		}
		loadFile.close();
